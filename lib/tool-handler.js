...
var TOOLS = [
  ...
  {
    name: 'element_log',
    description: 'Query the element activity log. Returns lifecycle events for discovered elements (discovered, registered, inference, tool calls, removals) with optional filtering.',
    inputSchema: {
      type: 'object',
      properties: {
        clientId: { type: 'string', description: 'Substring filter on element clientId' },
        type: { type: 'string', description: 'Filter by element type', enum: ['button', 'input', 'container'] },
        event: { type: 'string', description: 'Filter by event name (discovered, inference_start, inference_complete, registered, tool_called, tool_result, removed)' },
        url: { type: 'string', description: 'Substring filter on page URL' },
  ...
  },
  {
    ...
    description: 'Get a current snapshot of all registered element clients — status, type, page URL, inferred description, and tool call count for each.',
    inputSchema: { type: 'object', properties: {} }
  }
];
...
function handleCall(toolName, args, context) {
  ...
  if (toolName === 'element_log') {
    try {
      var elementLog = require('element-log');
      var filter = {
        clientId: args && args.clientId ? args.clientId : undefined,
        type: args && args.type ? args.type : undefined,
        event: args && args.event ? args.event : undefined,
        url: args && args.url ? args.url : undefined,
        limit: args && args.limit ? args.limit : 50
      };
      var entries = elementLog.query(filter);
      transport.step('element_log', 'ok', entries.length + ' entries returned');
      return { entries: entries, total: elementLog.size(), filter: filter };
    } catch (e) {
      return { error: 'element-log unavailable: ' + e.message };
    }
  }

  if (toolName === 'element_monitor') {
    try {
      var elementLog2 = require('element-log');
      var snap = elementLog2.snapshot();
      transport.step('element_monitor', 'ok', snap.total + ' element clients in snapshot');
      return snap;
    } catch (e) {
      return { error: 'element-log unavailable: ' + e.message };
    }
