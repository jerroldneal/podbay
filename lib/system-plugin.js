...
;(function () {
  ...
  // Tool name prefix: {name}.toolName when __podbayName is set
  var toolPrefix = (typeof __podbayName === 'string' && __podbayName) ? __podbayName + '.' : '';

  var tools = [
    {
      name: toolPrefix + 'execute_plugin',
  ...
  function handleToolCall(tool, args) {
    if (tool === toolPrefix + 'execute_plugin') {
