...
; (function () {
  ...
  // ── Plugin-registered tools (dynamic) ──────────────────────────────────
  // Plugins register tools via window.__podBay.registerTool()
  // Each entry: { name, description, inputSchema, handler }
  var pluginTools = {};

  function registerTool(name, description, inputSchema, handler) {
    if (!name || typeof handler !== 'function') {
      log('registerTool: name and handler are required');
      return false;
    }
    pluginTools[name] = {
      name: name,
    ...
    log('Plugin tool registered:', name);
    // Send updated tool list to broker
    sendToolUpdate();
    return true;
  }
  ...
  function unregisterTool(name) {
    if (pluginTools[name]) {
      delete pluginTools[name];
      log('Plugin tool unregistered:', name);
      sendToolUpdate();
      return true;
    }
    return false;
  ...
  function getAllTools() {
    var all = tools.slice();
    for (var k in pluginTools) {
      if (pluginTools.hasOwnProperty(k)) {
        var pt = pluginTools[k];
        all.push({ name: pt.name, description: pt.description, inputSchema: pt.inputSchema });
      }
    }
    return all;
  }
  ...
  function sendToolUpdate() {
    send({ type: 'update_tools', tools: getAllTools() });
  }
  ...
  function handleToolCall(tool, args) {
    ...
    if (tool === 'plugin') {
      return {
        ...
        pluginTools: Object.keys(pluginTools)
      };
    }
    // Check plugin-registered tools
    if (pluginTools[tool]) {
      try {
        return pluginTools[tool].handler(args || {});
      } catch (e) {
        return { error: e.message };
      }
    }
  ...
  function requestInjection() {
    ...
    xhr.onload = function () {
      try {
        ...
        injectionResult = { status: 'parse_error', error: e.message, timestamp: new Date().toISOString(), e };
        injected = true;
      }
    };
  ...
  window.__podBay = {
    ...
    listTools: function () { return Object.keys(pluginTools); }
  };
