...
function scheduleInferenceRetry(clientId, elementType) {
  ...
  setTimeout(function () {
    ...
    var retryPromise = __inferenceClient.inferElement(client.element, client.ws);
...
function createContainerClient(el) {
  ...
  ws.onopen = function () {
    ...
    var inferPromise = __inferenceClient ? __inferenceClient.inferElement(el, ws) : Promise.resolve(null);
  ...
  ws.onmessage = function (event) {
    ...
    if (msg.type === 'call_tool_result') {
      if (__inferenceClient && __inferenceClient.handleResult(msg)) return;
    }
