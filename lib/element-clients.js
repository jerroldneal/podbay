// PodBay Element Clients — DOM element observation and per-button broker clients
...
//
// Currently tracks: <button>, <input type="button|submit">, [role="button"]
...
function scanForButtons(root) {
  ...
  var selector = 'button, input[type="button"], input[type="submit"], [role="button"]';
  try {
    ...
    for (var i = 0; i < mutations.length; i++) {
      ...
      if (m.addedNodes) {
        for (var j = 0; j < m.addedNodes.length; j++) {
          ...
          if (isButton(added) && !added.__podbayClientId) createButtonClient(added);
      ...
      if (m.removedNodes) {
        for (var k = 0; k < m.removedNodes.length; k++) {
          ...
          if (isButton(removed) && removed.__podbayClientId) removeButtonClient(removed);
          // Also clean up child buttons
          try {
            var childBtns = removed.querySelectorAll('button, input[type="button"], input[type="submit"], [role="button"]');
