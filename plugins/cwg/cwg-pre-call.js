; (function () {
  ...
  if (window.__cwgPreCallInstalled) return;
  window.__cwgPreCallInstalled = true;

  var _bridge = (window.PodBayBridge && window.PodBayIdentity)
    ? window.PodBayBridge(window.PodBayIdentity.clientId) : null;
  if (!_bridge) { console.warn('[cwg-pre-call] PodBayBridge not available'); return; }
  ...
  _bridge.addTool(
    ...
    function (params) {
      ...
      return setPreAction(['prebetCall'], enabled);
    },
    'Set the pre-call toggle (enabled=true to arm, false to disarm)',
    { enabled: { type: 'boolean', description: 'true = arm call, false = disarm' } }
  );

  console.log('[cwg-pre-call] registered');
})();
